...
export function ArticleLayout({ article, children }: { article: Article; children: ReactNode }) {
  ...
  /* run-anywhere bundle: notebook + requirements + double-click launchers
     (.bat / .command / .sh), built by quant/build_bundles.py */
  const bundleHref = `/bundles/${article.slug}.zip`;
          ...
          <div className="min-w-0 py-16 lg:max-w-[46rem] lg:py-20">
            ...
            <aside className="corner-ticks mt-16 flex flex-col items-start justify-between gap-5 rounded-sm border border-anthracite/12 bg-paper/70 p-7 sm:flex-row sm:items-center">
              <div>
                ...
                <a href={notebookHref} download className="mt-2 inline-block t-mono text-[0.68rem] uppercase tracking-[0.14em] text-graphite underline-offset-4 hover:text-teal hover:underline">
                  or just the .ipynb ↓
                </a>
              </div>
              <a
