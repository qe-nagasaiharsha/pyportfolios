        ...
        <SubSection label="4.3" title="Express a view">
          ...
          <Figure caption="Figure 4.3 · One view on Germany moves every posterior — via correlation">
            <Bar
              ...
              series={[
                { name: "implied prior", values: d.returns.prior.map((v) => v * 100), color: "graphite" },
                { name: "posterior", values: d.returns.posterior.map((v) => v * 100), color: "teal" },
              ]}
              yFmt={{ decimals: 0, suffix: "%" }}
              xName="Market"
              yName="Expected return"
            />
          </Figure>
          <P>
        ...
        <SubSection label="4.4" title="Optimize on the blended returns">
          ...
          <Figure caption="Figure 4.4 · Allocations — market prior vs Black-Litterman vs naive MVO">
            <Bar
              ...
              labels={[...d.params.countries]}
              series={[
                { name: "market prior", values: d.weights.market.map((v) => v * 100), color: "graphite" },
                { name: "Black-Litterman", values: d.weights.blUnc.map((v) => v * 100), color: "teal" },
                { name: "naive MVO", values: d.weights.histUnc.map((v) => v * 100), color: "rust" },
              ]}
              yFmt={{ decimals: 0, suffix: "%" }}
              xName="Market"
